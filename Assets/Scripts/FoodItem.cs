...
public class FoodItem : MonoBehaviour
{
    ...
    public bool Push(Vector2Int direction)
    {
        ...
        if (levelManager.IsWallAt(nextPosition)) { return false; }
        FoodItem foodInNextPos = levelManager.GetFoodAt(nextPosition);
        if (foodInNextPos != null) { if (!foodInNextPos.Push(direction)) { return false; } }
    ...
    private void SetWorldPosition() { transform.position = grid.GetCellCenterWorld((Vector3Int)gridPosition); }
